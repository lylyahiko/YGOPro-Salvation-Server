...
function init(callback) {
    ...
    function attachCard(materialInfo, targetInfo) {
        var target = queryCard(targetInfo.player, targetInfo.location, targetInfo.index, targetInfo.overlayindex),
            material = queryCard(materialInfo.player, materialInfo.location, materialInfo.index, materialInfo.overlayindex);

        target.overlayUnits.push(materialUID);
        setState({
            player: material.player,
            clocation: material.location,
            index: material.index,
            moveplayer: target.player,
            movelocation: target.location,
            moveindex: target.index,
            moveposition: 'FaceUp',
            overlayindex: target.overlayUnits.length
        });
    }

    function detatchCard(targetInfo, index, moveInfo) {
        var target = queryCard(targetInfo.player, targetInfo.location, targetInfo.index, targetInfo.overlayindex),
            material = filterUID(stack, target.overlayUnits[index]);

            target.overlayUnits.splice(index, 1);
            
        setState({
            player: material.player,
            clocation: material.location,
            index: material.index,
            moveplayer: moveInfo.player,
            movelocation: moveInfo.location,
            moveindex: moveInfo.index,
            moveposition:moveInfo.position,
            overlayindex: moveInfo.overlayindex
        });
    }

    function giveMaterial(targetInfo, index, recieverInfo) {
        var target = queryCard(targetInfo.player, targetInfo.location, targetInfo.index, 0),
            reciever = queryCard(recieverInfo.player, recieverInfo.location, recieverInfo.index, 0),
            material = filterUID(stack, target.overlayUnits[index]),
            uid = material.uid;

            target.overlayUnits.splice(index, 1);
            attachCard(material, target);
    }
     ...
     * @param {Boolean} recursive Prevent the callback from firing.
     * @returns {undefined}
     */
    function setState(changeRequest, recursive) {
        ...
        var player = changeRequest.player,
            ...
            overlayindex = changeRequest.overlayindex || 0,
        ...
        stack[pointer].overlayUnits.forEach(function (materialUID, index) {
            var unit = filterUID(materialUID);
            setState({
                player: unit.player,
                clocation: unit.location,
                index: unit.index,
                moveplayer: moveindex,
        ...
        if (movelocation === 'GRAVE' || movelocation === 'REMOVED') {
            stack[pointer].overlayUnits = [];
        }

        cleanup = (movelocation == 'GRAVE') ? reIndex(moveplayer, 'GRAVE') : undefined;
        cleanup = (movelocation == 'HAND') ? reIndex(moveplayer, 'HAND') : undefined;
        cleanup = (movelocation == 'EXTRA') ? reIndex(moveplayer, 'EXTRA') : undefined;
        cleanup = (movelocation == 'EXCAVATED') ? reIndex(moveplayer, 'EXCAVATED') : undefined;

        cleanup = (clocation == 'GRAVE') ? reIndex(player, 'GRAVE') : undefined;
        cleanup = (clocation == 'HAND') ? reIndex(player, 'HAND') : undefined;
        cleanup = (clocation == 'EXTRA') ? reIndex(player, 'EXTRA') : undefined;
        cleanup = (clocation == 'EXCAVATED') ? reIndex(player, 'EXCAVATED') : undefined;
